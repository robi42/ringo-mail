...
/**
 * it sends mail from a google appengine application.
 * the inspiration was there documentation found here: http://code.google.com/appengine/docs/java/mail/usingjavamail.html
 * basically it is a short version of the above function send
 * Usage example:
 *
 *     mail.send({from: 'jdoe@example.com', to: 'hugo@example.com', text: 'Hi'});
 *
...
function gsend(data) {
    try {
        session = Session.getDefaultInstance(new java.util.Properties());
        var msg = createMsg(data);
        Transport.send(msg);
        log.info('Sent mail with following input data:', JSON.stringify(data));
    } catch (error) {
